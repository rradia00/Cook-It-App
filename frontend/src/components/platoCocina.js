...
export default function Mesa(argumentos) {
    ...
    function proceso(){
        var entregar = false;
        if(argumentos.estado==="green") entregar=true;
        return (
            (entregar) ? 
            ...
            <Container sx={{display: 'flex', flexDirection: 'colum'}}>
                ...
                <Grid container justifyContent="flex-end"> 
                    <Button sx={{color: 'white', width: '100', margin:"normal"}} 
                        onClick={() => {
                            argumentos.siguienteProceso(argumentos.posicion, argumentos.tipo);
                        }} 
                        variant="outlined">Siguiente proceso
                    </Button>
                </Grid>
            </Container>
        );
